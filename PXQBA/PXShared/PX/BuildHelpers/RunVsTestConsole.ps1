...
#{
#    & $vstestconsolepath $fakesArr '/inIsolation' '/logger:TeamCity' '/Platform:x64' '/UseVsixExtensions:true'
#}

# execute test DLLs without fakes with dotCover
if ($arr.length -gt 0)
{
    & $vstestconsolepath $arr '/inIsolation' '/Platform:x64' '/UseVsixExtensions:true'

    #build up command for vstest console to pass inside of dotCover command
    #$targetarguments = "/TargetArguments=" + $arr + " /inIsolation /logger:TeamCity /Platform:x64 /UseVsixExtensions:true"    
