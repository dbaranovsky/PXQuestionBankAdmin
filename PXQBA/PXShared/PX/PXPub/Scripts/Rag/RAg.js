...
// **********************************************************************
/*
    $(document).ready should start MySite_init, which ONLY sets up "loading" state UI and starts the RA init.
...
function MySite_init() {
    ...
    //$('#MySite_page_grayed').height( $(document).height() );
	...
	else if (window.location.host.substring(0, 4) == 'dev2') {
	    //MySite_BaseURL = 'dev.px.bfwpub.com/henrettaconcise4ev2/bcs/379';
	    MySite_BaseURL = 'dev.px.bfwpub.com/bps5e/lms/20';
	}
	else if (window.location.host.substring(0, 11) == 'dev-plugins') {
	//MySite_BaseURL = 'dev.px.bfwpub.com/bps5e/lms/20';
	    MySite_BaseURL = window.location.href;
	    MySite_BaseURL = MySite_BaseURL.replace('dev-plugins', 'dev');
	}
//	else
...
*/
	...
	var y = MySite_PageWin.document.getElementById('MySite_overlay_msg');
	var yInner = MySite_PageWin.document.getElementById('MySite_overlay_msgInner');
	var htmlMsg = '<div id="loadingBlock">Loading...</div>';
...
function MySite_RA_SetEventWireup(){
	$('body')[0].setAttribute('onunload','MySite_uninit()');
}
