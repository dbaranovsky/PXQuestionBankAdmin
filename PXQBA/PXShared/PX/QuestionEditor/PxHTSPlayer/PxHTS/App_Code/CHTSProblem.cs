...
namespace HTS
{
	...
	{
		private string[] unsSymbols = new string[]
		{
			"alpha",
			"beta",
			"gamma",
		...
		public void doProblem()
		{
			...
			{
				this.htsProblem = XDocument.Parse("<div>" + ex.Message + "</div>");
			}
		}
		private void DoCheckSymbols()
		{
			for (int i = 0; i < this.unsSymbols.Count<string>(); i++)
			{
				this.sProblem = this.sProblem.Replace("&" + this.unsSymbols[i] + ";", "&amp;" + this.unsSymbols[i] + ";");
			}
		}
		...
		private void processIproformula(XElement iproFormula)
		{
			...
			xElement.SetAttributeValue("style", CUtils.getFormulaStyle(array[1], isExpr, false));
		...
		private void processIproshort(XElement iproShort, CHTSIproStep step)
		{
			...
			string text = "";
			string text2 = this.baseUrl + "pictures/mathinput.gif";
			string text3 = "hts$QUESTIONID$." + stepID + ".EnterField" + cHTSIproshort._elid;
			string inputid = stepID + ".EnterField" + cHTSIproshort._elid;
			string text4 = "hts$QUESTIONID$." + stepID + ".anchor" + cHTSIproshort._elid;
			string text5 = "";
			...
			if (cHTSIproshort._type == "numeric" || cHTSIproshort._type == "text")
			{
				...
				{
					this.agilixAnswers.Add("getNumericAnswer('" + text3 + "')");
				}
				if (cHTSIproshort._type == "text")
				{
					this.agilixAnswers.Add("getTextAnswer('" + text3 + "')");
				}
				...
				else
				{
					...
					{
						string text6 = this.checkUserAnswer(iproShort, stepID) ? "success" : "fail";
						...
						string str = "</A>";
						xElement = XElement.Load(new StringReader(text7 + str));
					}
					else
					{
						if (this.p_showcorrect == "yes")
						{
							...
							string text9 = this.checkUserAnswer(iproShort, stepID) ? "success" : "fail_input";
			...
			{
				this.agilixAnswers.Add("getMathAnswer('" + text3 + "')");
				string str2 = "<A href=\"javascript:showEnterfieldAnswer('" + text3 + "');\" >";
				string str3 = "</A>";
				if (this.p_results == "show")
				{
					str2 = "";
				...
				else
				{
					if (this.p_showcorrect == "icononly")
					{
						string text13 = this.checkUserAnswer(iproShort, stepID) ? "success" : "fail";
						string text7 = string.Concat(new string[]
						{
							"<A id=\"",
							text4,
					...
					else
					{
						if (this.p_showcorrect == "yes")
						{
							...
							string text15 = this.checkUserAnswer(iproShort, stepID) ? "success" : "fail_input";
				...
				string text18 = " style=\"MARGIN-BOTTOM: -8px; VERTICAL-ALIGN: baseline\"";
				if (text != string.Empty)
				{
					text2 = this.baseUrl + "geteq.ashx?eqtext=" + HttpUtility.UrlEncode(text) + "&doborder=true&bottom=8";
					text18 = " style=\"" + CUtils.getFormulaStyle(text2, false, true) + "\"";
				}
			...
			try
			{
				...
				if (cHTSIproshort._type == "math")
				{
					...
					if (xElement3 != null)
					{
						xElement3.SetAttributeValue("src", text2);
					}
					else
					{
						xElement2.SetAttributeValue("src", text2);
					}
				}
		...
		private void setMCID(XElement mc)
		{
			...
			IEnumerable<XNode> enumerable2 = mc.Nodes();
			using (IEnumerator<XNode> enumerator2 = enumerable2.GetEnumerator())
			{
				while (enumerator2.MoveNext())
				{
					XElement content = (XElement)enumerator2.Current;
					mc.AddBeforeSelf(content);
				}
			}
			enumerable.Remove<XElement>();
		}
