...
//**MAD** procedure closeParkingWindows; //!AAL 12-20-03
procedure CloseParkingWindow;
procedure AxCtrls_IntelliproChanged; //!AAL 12-30-03

//!AAL - 123003 - each thread needs its own instance of this var so they don't step on each other
threadvar 
  xParkingWindow: HWND;

implementation

uses Consts, ComServ (* !AAL 01-05-04 - added for unique parking window name between controls *) ;
...
//!AAL - 123003 - thread safe list of all parking window handles of any ActiveForms
//**MAD**var
//**MAD**  parkingWindowList: TThreadList;
...
//        OutputDebugString(PChar(Format('AxCtrls: WM_NCDESTROY for ParkingWindow (%x) for %s',[wnd,ComServer.ServerName])));
        h := xParkingWindow;

        if h = Wnd then
          xParkingWindow := 0;
//        else if h <> 0 then
//          OutputDebugString(PChar(Format('AxCtrls: WM_NCDESTROY for ParkingWindow (%x), but stored Parking WIndow is {%x) for %s',[wnd,h,ComServer.ServerName])));
      end;
...
begin
  ...
  clName := 'DAXParkingWindow'+ComServer.ServerName;

  if h <> 0 then
  begin
    //Just in case check to make sure this is a valid window
    if IsWindow(h) and (GetWindowClassName(h) = clName) then
    begin
      Result := h;
      Exit;
    end
    ...
    begin
//      OutputDebugString(Pchar(Format('ParkingWindow handle (%x) is not valid for %s',[h,ComServer.ServerName])));
    end;
  end;

  // !AAL 01-05-04 - Added Comserver.ServerName to Class to prevent issue with MathXL Player parking window having same name
  // They don't share a window, but it was causes an issue opening wrapper after MathXL player regardless.  Weird.  Fix works though.
  FillChar(TempClass, sizeof(TempClass), 0);

//  OutputDebugString(Pchar(Format('%s: Calling GetClassInfo,hinstance=%x',[ComServer.ServerName,Hinstance])));

    //**MAD** Added code to check ParkingWindowProc.  This is for the case when the
    //the class doesn't get unregistered and mistakenly get's reused by a different
    //code space.
    classRegistered := GetClassInfo(HInstance, PChar(clName), TempClass);
    if not classRegistered or (TempClass.lpfnWndProc <> @ParkingWindowProc) then
    begin
      if classRegistered then
        Windows.UnregisterClass(PChar(clName),HInstance);
      TempClass.hInstance := HInstance;
      TempClass.lpfnWndProc := @ParkingWindowProc;
      TempClass.lpszClassName := PChar(clName);
      if Windows.RegisterClass(TempClass) = 0 then
        RaiseLastOSError;
    end;

(*
  if not GetClassInfo(HInstance, PChar(clName), TempClass) then
  begin
    TempClass.hInstance := HInstance;
    TempClass.lpfnWndProc := @ParkingWindowProc;
    TempClass.lpszClassName := PChar(clName);
...
*)
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Parking Window Create',[xParkingWindow])));
  h := CreateWindowEx(WS_EX_TOOLWINDOW, TempClass.lpszClassName, nil,
    WS_POPUP, GetSystemMetrics(SM_CXSCREEN) div 2,
    GetSystemMetrics(SM_CYSCREEN) div 2, 0, 0, 0, 0, HInstance, nil);
//  OutputDebugString(PChar(Format('AxCtrls (%x) - Parking Window Made for %s',[h,ComServer.ServerName]))); // !AAL 12-30-03 leave this one too
  SetWindowPos(h, 0, 0, 0, 0, 0, SWP_NOACTIVATE or SWP_NOREDRAW
    or SWP_NOZORDER or SWP_SHOWWINDOW);
  xParkingWindow := h;
//**MAD**  parkingWindowList.Add(Pointer(xParkingWindow)); //!AAL 12-30-03 - Add parking window reference to our list for freeing later
  Result := h;
end;
...
begin
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Initialize Start',[xParkingWindow])));
  inherited Initialize;
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Initialize After Inherited',[xParkingWindow])));
  ...
  InitializeControl;
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Initialize End',[xParkingWindow])));
end;

destructor TActiveXControl.Destroy;
begin
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Destroy Start',[xParkingWindow])));
  ...
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Destroy Before Inherited',[xParkingWindow])));
  inherited Destroy;
  //!AAL - 123003
  //OutputDebugString(PChar(Format('AxCtrls (%x) - Destroy End',[xParkingWindow])));
end;
...
begin
  ...
  begin
    ...
    case Msg of
      ...
      WM_MOUSEACTIVATE:
        begin
          //**MAD** This is just a test.  Netscape it handling the mouse activeate
          //and killing the focus on the current control.  So I'm going to try
          //to not call the normal stuff here.
          if FUIActive  then
...
//  OutputDebugString(Pchar(Format('Translate Accel:%d Message:%d, wparam:%d',[Result,msg.Message,msg.Wparam])));
end;


function TActiveXControl.OnFrameWindowActivate(fActivate: BOOL): HResult;
begin
  //Originally this method did not have the if fActivate and it was causing problems.
  if fActivate then
  begin
    Result := InPlaceActivate(True);
...
//!AAL - 123003
// * called from finalization which is called when last window hosting an ActiveForm is closed (!CLOSED!, not browsed to a different page)
// * added to close all parking windows that have been opened by ActiveForms
// * windows aren't necessarily freed by the window that opened them, so we need to send a close message to have them close
(****** MAD - I think we need to close the parking window from the DLL_DETACH_THREAD in DLLProc.
  no need for list anymore
procedure closeParkingWindows;
var
  parkingIterator: integer;
begin

  with parkingWindowList.LockList do
    try
      for parkingIterator:=0 to Count-1 do
      begin
        // AAL - 123003 - still send this one debug message.  Doesn't hurt to do just one.
        OutputDebugString(PChar(Format('AxCtrls (%x) - Parking Window SendCloseMsg for %s',[Integer(Items[parkingIterator]),ComServer.ServerName])));
        SendMessage(Integer(Items[parkingIterator]), WM_CLOSE, 0, 0);
      end;
    finally
      parkingWindowList.UnlockList;
    end;
end;
*)
...
var
  h : HWND;
begin
  //Since xParkingWindow is a threadver, debugger has trouble showing it
  //so load it into local var so we can look at it.
  h := xParkingWindow;
  if (h <> 0) and IsWindow(h) then
  begin
    //Only call Destroy windo if ParkingWindow handle is in this thread.
    if GetWindowThreadProcessId(h, nil) = GetCurrentThreadId then
    begin
//      OutputDebugString(PChar(Format('ParkingWindow (%x) - Closing parking window for %s',[h,ComServer.ServerName])));
      if IsWindow(h) then
        DestroyWindow(h);
    end
    else
    begin
//      OutputDebugString(PChar(Format('Attempting to close ParkingWindow (%x) from another thread (%x)',[h,GetCurrentThreadId])));
    end;
  end;
end;


// !AAL - Procedure is here so you get a compile error if you aren't using a fixed AxCtrls
procedure AxCtrls_IntelliproChanged;
begin
...
initialization
  ...
  TPicture.RegisterFileFormat('', '', TOleGraphic);
//**MAD**  parkingWindowList := TThreadList.Create; //!AAL - 123003
  InitOlePro32;



finalization
  // !AAL 12-30-03 - not harmful to leave
//**MAD**  OutputDebugString(PChar(Format('AxCtrls (%x) - AxCtrls Finalize for %s',[xParkingWindow,ComServer.ServerName])));
//**MAD**  closeParkingWindows;
//**MAD**  parkingWindowList.Free;
//**MAD** Since we're closing this parking window in the thread detach, we shouldn't need to close it here.
  CloseParkingWindow;
  if OlePro32DLL <> 0 then FreeLibrary(OlePro32DLL);
end.
