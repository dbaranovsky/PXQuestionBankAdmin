...
begin
  ...
  begin
    ...
    //**MAD** For DLLs  use Owner window handle if available 10/11/06
    if (hWndOwner = 0) or not Application.ShowMainForm then //This must be an activeX or Dll
    //**MAD** added or not ShowMainForm since EEB and MEB have handles.
    begin
      if Assigned(Owner) and (Owner is TWinControl) and TWinControl(Owner).HandleAllocated then
...
begin
  ...
  begin
    ...
    hWndOwner := Application.Handle;
    //**MAD** For DLLs  use Owner window handle if available 10/11/06
    if hWndOwner = 0 then //This must be an activeX or Dll
    begin
      if Assigned(Owner) and (Owner is TWinControl) and TWinControl(Owner).HandleAllocated then
        hWndOwner := TWinControl(Owner).Handle;
    end;
...
begin
  ...
  if Assigned(Screen.ActiveControl) then
    pForm := GetParentForm(Screen.ActiveControl)
  else
    pForm := nil;

  if Assigned(pForm) then
    Result := TMessageForm.CreateNew(pForm)
// Didn't like assigning parent are button or edit field.
//  if Assigned (Screen.ActiveControl) then                    //start change MEE dialogs DV 6/10/05
//    Result := TMessageForm.CreateNew(Screen.ActiveControl)
  else                                                       //end change
