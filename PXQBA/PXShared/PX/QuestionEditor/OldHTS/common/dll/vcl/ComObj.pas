...
uses ComConst, Contnrs, AxCtrls, Classes;

var
  OleUninitializing: Boolean;

//var
//  COMObjectList : TObjectList;
...
(* This is not quite working.  The Release section is messed up and
   IE is now giving me problems repeaing problem, so I'm shelving this idea
procedure CleanUpActiveForms();
var
 i : Integer;
 a : TActiveFormControl;
 list : TList;
begin

//  if not Assigned(ComObjectList) then
//    Exit;

  //Make separate list of items you want to free
  //because COMOBjectList will be updated while
  //it's happening
  list := nil;
  try
    for i := 0 to COMObjectList.Count-1 do
    begin
      if COMObjectList[i] is TActiveFormControl then
      begin
        a := TActiveFormControl(COMObjectList[i]);
        //Look for any controls left hanging.
        if (a.Control.ParentWindow <> 0) and not IsWindow(a.Control.ParentWindow) then
        begin
    ...
    begin
       ...
       begin
         a := TActiveFormControl(list[i]);
         //Just reduce the ref count so it will free itself.
         while (a.RefCount > 0) do
         begin
           a.ObjRelease;
         end;
       end;
...
//  if Assigned(ComOBjectList) then
//    COMObjectList.Add(Self);
end;
...
begin
  if Connecting then
  begin
//    OutputDebugString(Pchar('EventConnect:FAutoFactory.FEventIID='+GuidToString(FAutoFactory.FEventIID)));
...
//    OutputDebugString(pchar('FEventIID='+GUIDToString(FEventIID)));
...
//  ComObjectList := TObjectList.Create;
//  COMObjectList.OwnsObjects := False;
...
//  COMObjectList := nil;
  ...
  //**MAD** 1/15/06 - Nil ComClassMangerVar pointer so no one tries to use it.  Currently
  //ComServer is using it.
  ComClassManagerVar := nil;
