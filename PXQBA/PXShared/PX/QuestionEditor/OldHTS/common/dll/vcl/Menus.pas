...
//**MAD** make xPopupList a thread var so that it's unique for each thread
threadvar
   xPopupList : TPopupList;
...
//**MAD** create functio to return or create a popupList for this thread.
function PopupList:TPopupList;

//**MAD** this function should be call drom the DLL_THREAD_DETACH DLLProc.
procedure CleanupThreadPopupList;
...
//**MAD** Added to return or create a popuplist unique to current thread.
function PopupList:TPopupList;
begin
  if assigned(xPopupList) then
  begin
    Result := xPopupList;
    exit;
  end;
  xPopupList := TPopupList.Create;
  Result := xPopupList;
end;
...
begin
  if assigned(xPopupList) then
  begin
    if xPopupList.FWindow <> 0 then
    begin
      Classes.DeallocateHWnd(xPopupList.FWindow);
      xPopupList.FWindow := 0;
    end;
    xPopupList.Free;
  end;
end;
...
begin
  //Added check for null **MAD** 3/15/07
...
//**MAD** this is now created on the fly for each thread
//  PopupList := TPopupList.Create;
  ShortCutItems := TMenuItemStack.Create;

finalization
  //Inside EEB this item was getting free and then menus were going away and trying to access it
  //This might need to be a threadvar (like PopupList), but as a first cut, I'm going to try and nil it and
  //check for that in the problem area and we'll see if that's good enough.
  //ShortCutItems.Free;
  FreeAndNil(ShortCutItems);
  //**MAD** Changed to use threadvar for clean up
  //PopupList.Free;
  CleanupThreadPopupList;
  CommandPool.Free;
