...
begin
  ...
  begin
    ...
    try
      ...
      //**MAD** This should not happen, but I think it is.
      if FLockCount > 1 then
      begin
//        OutputDebugString('BitmapCanvas is locked by another. Trying again later');
        Exit;
      end;
      ...
      begin
        Handle := 0;
        ret := DeleteDC(H);
        //**MAD** Check return value if it's 0 then there was an error and we need
        //to leave this DC in the list so it will get cleaned up next time.
        if not ret then
        begin
          err := GetLastError;
          if (err = 0) or (FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, nil, err, LOCALE_USER_DEFAULT, Buf, sizeof(Buf), nil) = 0) then
            Buf := 'No Error Info';
          OutputDebugString(Pchar(Format('Could not Delete DC, lockcount=%d, error=%s',[FLockCount,buf])));
          Handle := H;
        end
        else
        begin
          BitmapCanvasList.Remove(Self);
          //OutputDebugString(Pchar(Format('Deleted DC Successfully, lockcount=%d',[FLockCount])));
        end;
      end
      else
        BitmapCanvasList.Remove(Self);
...
begin
  ...
  ScreenLogPixels := 96; //**MAD**120DPI Test
  ReleaseDC(0,DC);
