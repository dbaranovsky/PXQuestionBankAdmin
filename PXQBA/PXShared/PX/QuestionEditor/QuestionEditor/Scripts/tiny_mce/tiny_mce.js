	...
	b.create("tinymce.ui.Button:tinymce.ui.Control", {
		...
		postRender: function ()
		{
			var c = this,
                ...
                d;
			if (b.isIE && c.editor) {
				b.dom.Event.add(c.id, "mousedown", function (f)
				{
					d = c.editor.selection.getBookmark()
				})
			}
			b.dom.Event.add(c.id, "click", function (f)
			{
				if (!c.isDisabled()) {
					if (b.isIE && c.editor && d) {
						b.activeEditor.selection.moveToBookmark(d)
					}
					return e.onclick.call(e.scope, f)
				}
			});
			b.dom.Event.add(c.id, "keyup", function (f)
			{
				if (!c.isDisabled() && f.keyCode == b.VK.SPACEBAR) {
					return e.onclick.call(e.scope, f)
				}
			})
		}
...
(function (n)
{
	...
	n.create("tinymce.Editor", {
		...
		_addEvents: function ()
		{
			...
			C.onKeyDown.add(function (t, E)
			{
				if (E.keyCode != j.BACKSPACE) {
					return
				}
				...
				var G = s.startContainer;
				var F = s.startOffset;
				while (G && G.nodeType && G.nodeType != 1 && G.parentNode) {
					G = G.parentNode
				}
				if (G && G.parentNode && G.parentNode.tagName === "BLOCKQUOTE" && G.parentNode.firstChild == G && F == 0) {
					t.formatter.toggle("blockquote", null, G.parentNode);
					s.setStart(G, 0);
					s.setEnd(G, 0);
			...
			if (D.custom_undo_redo) {
				...
				C.dom.bind(C.dom.getRoot(), "dragend", function (s)
				{
					z()
				});
				C.onKeyUp.add(function (s, E)
				{
					var t = E.keyCode;
					if ((t >= 33 && t <= 36) || (t >= 37 && t <= 40) || t == 13 || t == 45 || E.ctrlKey) {
						z()
					}
				});
				C.onKeyDown.add(function (s, F)
				{
					var E = F.keyCode,
                        t;
					if (E == 8) {
					...
					if ((E >= 33 && E <= 36) || (E >= 37 && E <= 40) || E == 13 || E == 45) {
						if (n.isIE && E == 13) {
							C.undoManager.beforeChange()
						}
					...
					if ((E < 16 || E > 20) && E != 224 && E != 91 && !C.undoManager.typing) {
						C.undoManager.beforeChange();
	...
	a.Formatter = function(V) {
		...
		function o(W, ai, Z) {
			...
			function aj(ar) {
				...
				for (;;) {
					if (ap == root || (!ai[0].block_expand && F(ap))) {
						return ap
					}
		...
		function Q(af, W, ad) {
			var ai, ag = "_mce_caret",
			    X = V.settings.caret_debug;
			ai = a.isGecko ? "\u200B" : E;
