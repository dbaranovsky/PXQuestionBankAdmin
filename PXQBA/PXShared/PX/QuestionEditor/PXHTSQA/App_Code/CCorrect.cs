...
namespace HTS
{
	public class CCorrect
	{
		...
		internal double getTolerance(bool bForPopup)
		{
			...
			try
			{
				result = Convert.ToDouble(this._tolerance) + (bForPopup ? 0.0 : 1E-10);
			}
			catch (Exception)
			{
				try
				{
					result = Convert.ToDouble(this._tolerance.Replace(".", ",")) + (bForPopup ? 0.0 : 1E-10);
				}
				catch
				{
				}
		...
		public bool checkAnswer(string uAnswer)
		{
			...
			if ((type = this._type) != null)
			{
				...
				{
					double tolerance = this.getTolerance(false);
					uAnswer = uAnswer.ToUpper();
