...
%>
<!--#include virtual="/RA/RAXS/v1.2/RAXS_server.asp"-->
<%
...
dim gourl
if iUserID<>"" and iUserID<>0 then
		gourl = RALoginRefURL &"?m=i&returl="& Server.URLEncode( returl ) &"&u="&iUserID
elseif iUserID=0 then
		gourl = RALoginRefURL &"?m=o&returl="& Server.URLEncode( returl ) &"&u="&iUserID
else
		gourl = RALoginRefURL &"?m=c&returl="& Server.URLEncode( returl ) &"&u="&iUserID
end if
if debug then
	response.write "<a href="""& gourl &""">"& gourl &"</a>"
else
