...
if show = "none" then
	response.redirect "http://bcs.bfwpub.com/login_reference/asp/c-ra.asp?m=i&returl="&Server.URLEncode(returl)
	response.end
else
%>
<html>
...
setTimeout('window.opener.location = \'http://bcs.bfwpub.com/login_reference/asp/c-ra.asp?m=i&u=<%=iUserID%>&returl=<%=Server.URLEncode(returl)%>\'', 50);
</script>
<%
end if
...
sub RAWS_Http_Request (wsF, strEnvelope)
	...
	dim wsURL : wsURL = "http://"& RARootDomain &"/ra_webservices/webservice1/service1.asmx?wsdl"
...
"	tblUserAssignments.Expiration "&_
"FROM         tblUserAssignments WITH (nolock) "&_
"             INNER JOIN  tblPackage WITH (nolock) ON tblUserAssignments.PackageID = tblPackage.PackageID "&_
"WHERE     (tblUserAssignments.UserID = "& CLng(RACU_iUserID) &") "&_
"ORDER BY tblUserAssignments.Expiration DESC"&_
""
