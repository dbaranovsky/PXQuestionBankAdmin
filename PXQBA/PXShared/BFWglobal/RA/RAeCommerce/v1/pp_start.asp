...
for i=1 to 30
	if Request.Form("item_name_"&i) <> "" AND Request.Form("PackageID_"&i) <> "" AND Request.Form("amount_"&i) <> "" then
		...
		strQuery = "EXEC ra_ecomm_recordTransaction @userid="& CLng(iUserID) &""&_
		", @sid="& CLng(iSiteID) &""&_
		", @pid="& CLng(xPackageID) &""&_
		", @price='"& xPackagePrice&"'"&_
		", @date='"& xDate &"'"&_
		", @confirmed=0"&_
		", @canceled=0"&_
		", @schoolid="& CLng(iSchoolID) &""&_
		", @schoolzip='"& iSchoolZip &"'"&_
		", @schoolname='"& sSchoolName &"'"&_
		", @schooltype='"& sSchoolType &"'"&_
		""
...
function init () {
	if (!window.opener) {
		...
		pp_return.value = 'http://'+ window.location.hostname + '/RA/RAeCommerce/v1/pp_return.asp?returl='+encodeURIComponent(returl)+'&cancelurl='+encodeURIComponent(cancelurl);
		var pp_cancel_return = document.getElementById('pp_cancel_return');
		pp_cancel_return.value = 'http://'+ window.location.hostname + '/RA/RAeCommerce/v1/pp_cancel.asp?returl='+encodeURIComponent(returl)+'&cancelurl='+encodeURIComponent(cancelurl);
