...
//alert('RAWS_HTTPREQUEST  :::::: '+ RAWS_function);
	...
	switch (RAWS_function) {
		case 'RAXS_WS' :
	szUrl = '/BFWglobal/ws/proxy.asp?wsID=RAXS_WS';
			break;
		case '2_CheckUsername' :
	...
	RA_CtrlWin.RAWS_Http = $.ajax({
		...
		beforeSend: function(req) {
			...
			req.setRequestHeader('SOAPAction', 'http://tempuri.org/'+ RAWS_function) ;
		},
		success: function(data,textStatus) {
			RA_CtrlWin.RAWS_XML = data;
//alert(data);
//alert(textStatus);
//prompt('Content-Length',RA_CtrlWin.RAWS_Http.getResponseHeader('Content-Length'));
//alert(RA_CtrlWin.RAWS_Http.getAllResponseHeaders());
			cbk();
		},
		error: RAWS0_ERROR
	});
...
function RAWS_HTTPREQUEST_X (RAWS_function, strEnvelope) {
//alert('RAWS_HTTPREQUEST  :::::: '+ RAWS_function);
//try {
...
//prompt('RA_CtrlWin.RAWS_XML.xml',$('RAXS_WS',RA_CtrlWin.RAXS_WS_XML) );
//prompt('RA_CtrlWin.RAWS_XML.xml',$('RAXS_WS:first > results:first > udtUserProfile:first > iUserID',RA_CtrlWin.RAWS_XML).text() );
if (0) {
		var $this = $('RAXS_WS:first > results:first > udtUserProfile',RA_CtrlWin.RAWS_XML);
		var len = $this.length;
		if (len > 0) {
			RA_CtrlWin.RAWS_iUserID = $('RAXS_WS:first > results:first > udtUserProfile:first > iUserID',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sUserName = $('RAXS_WS:first > results:first > udtUserProfile:first > sUserName',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sPassword = $('RAXS_WS:first > results:first > udtUserProfile:first > sPassword',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sFirstName = $('RAXS_WS:first > results:first > udtUserProfile:first > sFirstName',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sLastName = $('RAXS_WS:first > results:first > udtUserProfile:first > sLastName',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sPasswordHint = $('RAXS_WS:first > results:first > udtUserProfile:first > sPasswordHint',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sMailPreference = $('RAXS_WS:first > results:first > udtUserProfile:first > sMailPreference',RA_CtrlWin.RAWS_XML).text();
			RA_CtrlWin.RAWS_sOptInEmail = $('RAXS_WS:first > results:first > udtUserProfile:first > sOptInEmail',RA_CtrlWin.RAWS_XML).text();
		}
//prompt('RA_CtrlWin.RAWS_XML.xml',$('RAXS_WS:first > results:first',RA_CtrlWin.RAWS_XML).text() );
var htmlMsg = '';
		$this;
		$this = $('RAXS_WS:first > results:first > udtClass',RA_CtrlWin.RAWS_XML);
		len = $this.length;
htmlMsg = len +'::::::::';
		for (var i=0; i<len; i++) {
htmlMsg += '\n'+ i +' : ';
htmlMsg += $this.filter(':first').children('sCreatorLName:first').text();
htmlMsg += ' : ';
htmlMsg += $this.filter(':first').children('iCreatorID:first').text();
htmlMsg += ' : ';
htmlMsg += $this.filter(':first').children('sCreatorEmail:first').text();
htmlMsg += ' : ';
htmlMsg += $this.filter(':first').children('sClassCode:first').text();
htmlMsg += ' : ';
htmlMsg += $this.filter(':first').children('iUserID:first').text();
htmlMsg += ' : ';
htmlMsg += $this.filter(':first > bEmailScores').text();
			RA_CtrlWin.RAWS_udtClassInfo[i] = new Array;
			RA_CtrlWin.RAWS_udtClassInfo[i]['iClassID'] = $this.filter(':first').children('iClassID').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['iCreatorID'] = $this.filter(':first').children('iCreatorID').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['sClassName'] = $this.filter(':first').children('sClassName').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['sClassDesc'] = $this.filter(':first').children('sClassDesc').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['sClassCode'] = $this.filter(':first').children('sClassCode').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['dtExprn'] = $this.filter(':first').children('dtExprn').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['iUserID'] = $this.filter(':first').children('iUserID').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['bClassAccessRevoked'] = $this.filter(':first').children('bClassAccessRevoked').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['dtLastLogin'] = $this.filter(':first').children('dtLastLogin').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['dtStartDate'] = $this.filter(':first').children('dtStartDate').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['dtEndDate'] = $this.filter(':first').children('dtEndDate').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['bEmailScores'] = $this.filter(':first').children('bEmailScores').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['iRecordStatus'] = $this.filter(':first').children('iRecordStatus').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['sCreatorEmail'] = $this.filter(':first').children('sCreatorEmail').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['sCreatorFName'] = $this.filter(':first').children('sCreatorFName').text();
			RA_CtrlWin.RAWS_udtClassInfo[i]['sCreatorLName'] = $this.filter(':first').children('sCreatorLName').text();
...
*/
		RA_CtrlWin.RAWS_iUserID = $('UserAuthenticationResult', RA_CtrlWin.RAWS_XML).text();
	}
