...
//INSTRUCTOR EMAIL VERSION
			RA_CtrlWin.RA.RAif.state.push('autologgingin');
			RA_DrawPage();
			RA_AutoLogin();
		break;
		case 'quizprompt' :
...
function RA_goLogin () {
	RA_CtrlWin.RA.RAif.vars['Email'] = cleanInput($('#RA_Email')[0].value);
	RA_CtrlWin.RA.RAif.vars['Password'] = cleanInput($('#RA_Password')[0].value);
...
function RA_goCheckEmailANDLogin () {
	...
	RA_CtrlWin.RA.RAif.vars['Email'] = cleanInput($('#RA_EmailLOGIN')[0].value);
...
//	RA_CtrlWin.RA.RAif.vars['Email'] = cleanInput($('#RA_Email')[0].value);
	RA_CtrlWin.RA.RAif.vars['Email_confirm'] = cleanInput($('#RA_Email_confirm')[0].value);
	RA_CtrlWin.RA.RAif.vars['FName'] = cleanInput($('#RA_FName')[0].value);
	RA_CtrlWin.RA.RAif.vars['LName'] = cleanInput($('#RA_LName')[0].value);
	RA_CtrlWin.RA.RAif.vars['Password'] = cleanInput($('#RA_Password')[0].value);
	RA_CtrlWin.RA.RAif.vars['Password_confirm'] = cleanInput($('#RA_Password_confirm')[0].value);

	var xHint = cleanInput($('#RA_Password')[0].value);
...
function RA_goCheckCode () {
	...
	RA_CtrlWin.RA.RAif.vars['Code'] = cleanInput($('#RA_Code')[0].value);
...
//window.self.focus();
	...
	var z = new Number( cleanInput($('#RA_iZip')[0].value.replace('-','')) );
	if (isNaN(z)) {
...
//window.self.focus();
	...
	RA_CtrlWin.RA.RAif.vars['iZip'] = cleanInput($('#RA_iZip')[0].value.replace('-',''));
...
//window.self.focus();
	...
	RA_CtrlWin.RA.RAif.vars['sUserEmail'] = cleanInput($('#RA_sUserEmail')[0].value);
	RA_CtrlWin.RA.RAif.vars['sInstructorEmail'] = '';
	RA_CtrlWin.RA.RAif.vars['sUserFirstName'] = cleanInput($('#RA_sUserFirstName')[0].value);
	RA_CtrlWin.RA.RAif.vars['sUserLastName'] = cleanInput($('#RA_sUserLastName')[0].value);
...
//	RA_CtrlWin.RA.RAif.vars['sOptInEmail'] = 0;
	...
	RA_CtrlWin.RA.RAif.vars['sUserName'] = cleanInput($('#RA_sUserEmail')[0].value);
	RA_CtrlWin.RA.RAif.vars['sOldPwd'] = cleanInput($('#RA_sOldPwd')[0].value);
	RA_CtrlWin.RA.RAif.vars['sNewPwd'] = cleanInput($('#RA_sNewPwd')[0].value);
	RA_CtrlWin.RA.RAif.vars['sVPwd'] = cleanInput($('#RA_sVPwd')[0].value);
	var xHint = cleanInput($('#RA_sNewPwd')[0].value);
	xHint = xHint.substring(0,1) +' '+ xHint.substring(1,xHint.length-2) +' '+  xHint.substring(xHint.length-2);
	RA_CtrlWin.RA.RAif.vars['sHintPwd'] = xHint;
	RA_UpdateProfile();
}
...
//alert(RA_CtrlWin.RA.RAif.vars['sInstructorEmail'] +' --- '+ cleanInput($('#RA_sInstructorEmail')[0].value));
	RA_CtrlWin.RA.RAif.vars['sInstructorEmail'] = cleanInput($('#RA_sInstructorEmail')[0].value);
...
function RA_goQuizJoinClass () {
	RA_CtrlWin.RA.RAif.vars['sClassCode'] = cleanInput($('#RA_sClassCode')[0].value);
	RA_QuizJoinClass();
}
...
//alert(RA_CtrlWin.RA.CurrentUser.ClassUsing);
	...
	if (window.location.host=='192.168.77.114') {
...
function RA_AutoLogin () {
//alert('DONE auth');
	RA_CtrlWin.RA.RAif.state.pop();
	RA_CtrlWin.RA.RAif.state.push('autologgedin');
	var gourl = 'http://'+ RA_CtrlWin.RA.RAXSURL +'/RAXS_Login.asp?uid='+RA_CtrlWin.RA.RAif.vars['auto login id']+'&sid='+RA_CtrlWin.RA.CurrentSite.ID+'&pids=&returl='+ top.location.href;
alert(gourl);
//	RA_PopUpGlobal_Go(gourl);
	top.location = gourl;
}
