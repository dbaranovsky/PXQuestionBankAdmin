...
function RA_goEditInstEmail() {
	RA_CtrlWin.RA.RAif.state.push('instemailprompt');
	RA_DrawPage();
}

function RA_goInstEmailPrompt () {
...
function RA_CheckInstEmail() {
	RA_CtrlWin.RA.RAif.state.pop();
	RA_CtrlWin.RA.RAif.state.push('instemailprompted');
	RA_CtrlWin.RA.RAif.state.push('checkinginstemail');
	RA_DrawPage();
	RA_CtrlWin.RA.RAWS.Send( RA_CheckInstEmail_0_go, RAWS1_GetUsernamePasswordHint, new Array( 'sUserName='+RA_CtrlWin.RA.RAif.vars['sInstructorEmail'], 'sBaseUrl=' ) );
}

function RA_CheckInstEmail_0_go () {
	RA_CheckInstEmail_0();
}

function RA_CheckInstEmail_0 () {
...
function RA_BackToCart (clearError) {
	RA_CtrlWin.RA.RAif.state.pop();
	RA_CtrlWin.RA.RAif.state.push('cart');
	if (clearError) {
		RA_CtrlWin.RA.RAif.vars['Error'] = '';
...
function RA_UpdateProfile () {
	...
	RA_DrawPage();
	if (useRAWS3) {
		RA_CtrlWin.RA.RAWS.Send( RA_UpdateProfile_0_go, RAWS3_UpdateProfile, new Array( 'iUserID='+RA_CtrlWin.RA.RAif.vars['iUserID'], 'sUserEmail='+RA_CtrlWin.RA.RAif.vars['sUserEmail'], 'sInstructorEmail='+RA_CtrlWin.RA.RAif.vars['sInstructorEmail'], 'sUserFirstName='+RA_CtrlWin.RA.RAif.vars['sUserFirstName'], 'sUserLastName='+RA_CtrlWin.RA.RAif.vars['sUserLastName'], 'sBaseURL='+RA_CtrlWin.RA.RAif.vars['sBaseURL'], 'sOptInEmail='+RA_CtrlWin.RA.RAif.vars['OptInEmail'] ) );
	} else {
...
//					if (item.InCart) {
		...
		case 'checkoutschoolprompt' :
			...
			str += '<td width="170" class="RA_button">'+ ButtonHTML('BACK TO MY SHOPPING CART', 'JavaScript:RA_BackToCart(true)', '', 'primary', false, 'medium', 'style="float:none; text-align:center"') +'</td>';
			str += '<td width="60" class="RA_button">'+ ButtonHTML("CANCEL", "JavaScript:RA_FormCancel()", "", "primary", false, "medium", "style='float:none; text-align:center'") +'</td>';
			str += '<td>&nbsp;</td>';
			str += '</tr></table>';
		break;
	...
	//  ********************************************************************************
		case 'updateprofile' :
			...
			if (RA_CtrlWin.RA.CurrentUser==null) {
				...
				str += '&nbsp;&nbsp;&nbsp;&nbsp;<a class="RA_editlink" href="Javascript:RA_goEditInstEmail();">edit</a>';
				str += '</td>';
				str += '<td valign="top">&nbsp;</td>';
				str += '</tr><tr>';
				str += '<td valign="top">&nbsp;</td><td valign="top">&nbsp;</td><td valign="top">&nbsp;</td>';
...
//alert(RA_CtrlWin.RA.CurrentUser.ClassLogins.length);
			...
			str += '<p></p>';
			str += '<form name="RA_InstEmailPrompt" action="JavaScript:RA_goInstEmailPrompt();">';
			str += '<table width="100%" border="0" cellpadding="0" cellspacing="0"><tr>';
			...
			str += RA_CtrlWin.RA.CurrentUser.SiteLogins[RA_CtrlWin.RA.CurrentSite.ID].InstructorEmail;
			str += '"/>&nbsp;&nbsp;</nobr></td>';
			str += '<td width="30" class="RA_button">'+ ButtonHTML("GO", "JavaScript:RA_goInstEmailPrompt();", "", "primary", false, "medium", "style='float:none; text-align:center'") +'</td>';
			str += '<td>&nbsp;</td>';
			str += '</tr></table>';
			str += '<input type="submit" name="submit" style="position:absolute;top:-500px;left:-500px"/></form>';
			str += '<br/>';
			str += '</div>';
			str += '<table class="RA_buttons" border="0" cellpadding="0" cellspacing="0"><tr>';
			str += '<td width="60" class="RA_button">'+ ButtonHTML("CANCEL", "JavaScript:RA_FormContinue()", "", "primary", false, "medium", "style='float:none; text-align:center'") +'</td>';
			str += '<td>&nbsp;</td>';
			str += '</tr></table>';
		break;		
	//  ********************************************************************************
...
//alert('RAif_RAXS_Inited');
	...
	RA_CtrlWin.RA.RAif._if.src = '/BFWglobal/RAg/v1.4/RAif.html?clear=true';
}
