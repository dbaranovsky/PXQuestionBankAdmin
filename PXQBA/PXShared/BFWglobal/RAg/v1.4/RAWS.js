...
function RAWS0_ERROR() {
	if (RA_CtrlWin.RAWS_Http.status != 0 ) {
		alert('An error has occurred and the server is unable to process your request at this time. Please try again. [error code: '+ RA_CtrlWin.RAWS_Http.status +']');
		top.location.reload();
	} else {
//		alert('YO');
	}
...
if (RA_CtrlWin.RA.dev_check()) prompt('RA_CtrlWin.RAWS_XML', $('UpdateProfileResult',RA_CtrlWin.RAWS_XML).text());
...
*/
		result = $('EmailLoginsResult', RA_CtrlWin.RAWS_XML).text();
