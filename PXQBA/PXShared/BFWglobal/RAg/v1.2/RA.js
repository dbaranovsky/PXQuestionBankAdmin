...
*********************************************************
 ...
 REQUIRES JS:
	...
	<script src="/BFWglobal/RAg/v1.2/RAWS.js" type="text/javascript"></script>
...
function RAObj (
	...
	case '192.168.77.114' :
		...
		this.RAXSURL = this.RARootDomain +'/RA/RAXS/v1.2'
...
//alert(window.location.host);
	...
	if (window.location.host.indexOf('int-')==0)
	{
...
//alert(RAif_div);
			...
			this.win.$('#RAif')[0].src = '/BFWglobal/RAg/v1.2/RAif.html?url='+ encodeURIComponent(RA_CtrlWin.location.href);
...
if (RA_CtrlWin.RA.dev_check('raifstate')) alert(this.state.last);
			...
			if (this.updated_needsRAXS) {
				RA_CtrlWin.RA.SaveSession();
				this.Clear();
if (RA_CtrlWin.RA.dev_check()) prompt('testing','');
try {
				var xhref = RA_GetRAXScheck_url();
				var gourl = '';
...
if (RA_CtrlWin.RA.dev_check()) prompt('1, RAXScheck_url',gourl);
				} else {
			gourl = 'http://'+ RA_CtrlWin.RA.OldLoginRefURL +'/asp/c-ra.asp?m=o&returl='+ encodeURIComponent('http://'+xhref);
if (RA_CtrlWin.RA.dev_check()) prompt('2, RAXScheck_url',gourl);
if (RA_CtrlWin.RA.dev_check()) prompt('2, cookie',getCookie2('RAXScheck_url'));
				}
				if (xhref!='') {
if (RA_CtrlWin.RA.dev_check()) prompt('3, going to', gourl);
if (RA_CtrlWin.RA.dev_check()) prompt('3, cookie',getCookie2('RAXScheck_url'));
					setTimeout('top.location.href=\''+ gourl+'\'',200);
				} else {
if (RA_CtrlWin.RA.dev_check()) prompt('3, xhref blank', xhref);
				}
...
//msg2 += '\n SA '+ SA.Site.ID;
			...
			for (var iP=0; iP<RAWS_udtSites[iS].Products.length; iP++) {
				...
				case 'RA PACKAGE' :
					...
					P = RA_CtrlWin.RA.AddProduct( RAWS_udtSites[iS].Products[iP].ProductID, RAWS_udtSites[iS].Products[iP].Title, RAWS_udtSites[iS].Products[iP].Type, PS );
					...
					for (var isite in P.TypeObj.SiteAssignments) {if (P.TypeObj.SiteAssignments.hasOwnProperty(isite)) {
						...
						var x = RA_CtrlWin.RA.AddProduct( tmpID, P.TypeObj.SiteAssignments[isite].Site.Description, 'RA PACKAGE SITE', P.TypeObj.SiteAssignments[isite].Site );
...
//alert(cstr);
					...
					break;
				case 'RA SITE' :
//					PS = RA_CtrlWin.RA.AddSite( RAWS_udtSites[iS].Products[iP].ProductID, RAWS_udtSites[iS].Products[iP].BaseURL, '' );
...
RAObj.prototype.DisplayAll = function(
	...
	if (!RA_CtrlWin.RA.dev_check()) return '';
...
*/
	...
	str += '<a href="JavaScript:RA_CtrlWin.RA.RAif.init(\'quizclassprompt\')">Enter instructor e-mail</a><br/>';
...
//	if (!RA_CtrlWin.RA.dev_check()) return '';
	...
	str += '<tr><td valign="top" width="10"><nobr>TypeObj = </nobr></td><td valign="top">(Package Site, ID='+this.TypeObj.ID +')</td></tr>';
		break;
		case 'RA PACKAGE' :
	str += '<tr><td valign="top" width="10"><nobr>TypeObj = </nobr></td><td valign="top">(Package, ID='+this.TypeObj.ID +')</td></tr>';
		break;
		default :
...
RAObj.prototype.dev_check = function ( val ) {
	if (val) {
		if (RA_CtrlWin.RA.dev==val) return true
	} else {
