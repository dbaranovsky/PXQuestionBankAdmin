...
namespace BLTI
{
	public class OAuthBase
	{
		...
		private RequestParameters GetRequestParameters(string parameters)
		{
            ...
            // Parameters must be decoded per OAuth RFC 5849 paragraph 3.4.1.3.1, sbd(1)
            // Note: Assessed using HttpUtility::ParseQueryString as it seemed a good fit,
            // but concluded it cannot be used as multiple occurrences of the same 
            // query string parameter are listed as a single entry with a comma separating each value.
            // OAuth wants them each listed separately.
			...
			if (!string.IsNullOrEmpty(parameters))
			{
				...
				foreach (string s in p)
				{
					if (!string.IsNullOrEmpty(s) && !s.StartsWith(OAuthParameterPrefix))
					{
						...
						{
							string[] temp = s.Split('=');
							result.Add(new RequestParameter(HttpUtility.UrlDecode(temp[0]), HttpUtility.UrlDecode(temp[1])));   // sbd(1)

						}
						else
						{
							result.Add(new RequestParameter(HttpUtility.UrlDecode(s), string.Empty));   // sbd(1)
						}
					}
				}
			}
		...
		/// <summary>
        ////Note: this function only comples with RFC 3986 URL encoding spec (used for OAuth) in .NET 4.5+ and the following is set:
        ///<uri>
        ///  <idn enabled="All" />
        ///  <iriParsing enabled="true" />
        ///</uri>
        /// Standard URL encoding cannot be used because it uses lower case characters
		/// </summary>
        ...
        ///The below should not be necessary but it looks like Uri.EscapeDataString still does not comply with RFC 3986 spec
        /// <summary>
		...
		/// <returns>A base64 string of the hash value</returns>
		public string GenerateSignature(Dictionary<string, string> signatureParams, Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret,
													string httpMethod, string timeStamp, string nonce, SignatureTypes signatureType, out string signatureBase)
		{
			signatureBase = "";
		...
		private Dictionary<string, string> ReturnPostandHeaderParameters(HttpRequest httpRequest)
		{
			...
			foreach (var splitOauthParam in splitOauthParams)
			{
				...
				//add only if it starts with oauth_
				if (key.StartsWith(OAuthParameterPrefix))
